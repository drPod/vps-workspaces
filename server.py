#!/usr/bin/env python3
"""Workspace sharing-link authentication and layout metadata; Caddy owns transport."""
...
from model import surfaces, browser_host, local_browser
...
async def handle(req):
    ...
    if browser is None and req.path == "/login":
        raise web.HTTPFound("/")
    if req.path == "/auth":
        # Caddy overwrites this header from the original request before the auth subrequest.
        if req.headers.get("X-VWS-Upgrade", "").lower() == "websocket":
