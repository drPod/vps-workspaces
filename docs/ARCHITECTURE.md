...
Caddy terminates HTTPS and connects to an aiohttp Unix socket through an existing bind mount. ttyd uses separate private Unix sockets: no public ttyd listener. The web adapter checks a signed, expiring workspace cookie before serving the layout, terminal HTTP, terminal WebSockets, or local app proxies. A stable 256-bit per-workspace link key is derived from the installation secret. It travels in the URL fragment (not HTTP request URLs or access logs), is exchanged for a session cookie, and is removed from visible history before loading child frames. Secrets and relay records are outside the source tree with restrictive permissions.
