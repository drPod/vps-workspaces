...
from vps_workspaces.model import browser_host, local_browser, origin, surfaces


def preview_id(surface: Surface) -> str:
...
def render(documents: list[Workspace]) -> str:
    blocks = []
...
def prepare_previews(documents: list[Workspace]) -> None:
    ...
    if wanted:
        subprocess.run(
            ["systemctl", "--user", "enable", "--now", *sorted(wanted)],
            check=True,
