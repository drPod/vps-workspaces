...
from vps_workspaces.contracts import BackupConfig, BackupEntry
...
def sqlite_copies(roots: list[Path], stage: Path) -> list[BackupEntry]:
    manifest: list[BackupEntry] = []
    for root in roots:
        for directory, dirs, files in os.walk(root):
            ...
            for name in files:
                ...
                def progress(
                    status: int, remaining: int, total: int, deadline: float = deadline, source: Path = source
                ) -> None:
    ...
    current = {Path(entry["snapshot"]) for entry in manifest if "snapshot" in entry}
    for previous in stage.rglob("*.backup"):
...
def main() -> None:
    ...
    config: BackupConfig = json.loads(config_path.read_text())
        ...
        if config.get("remote_ready"):
            subprocess.run(
                [
                    ...
                    "BatchMode=yes",
                    config["ssh_host"],
                    "test -f " + shlex.quote(config["remote_ready"]),
                ],
                check=True,
            )
            ...
            json.dumps(
                {
                    ...
                    "sqlite_databases": len(manifest),
                    "warnings": [d for d in manifest if "error" in d],
                },
                indent=2,
            )
        )
