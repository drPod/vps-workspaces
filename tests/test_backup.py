    ...
    def test_first_snapshot_keeps_all_sources_and_next_snapshot_keeps_history(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for source in sources:
                source.mkdir(); (source / 'saved.txt').write_text('original')
            excludes = state / 'exclude.txt'; excludes.touch()
            conf = state / 'rsnapshot.conf'
            rows = [('config_version', '1.2'), ('snapshot_root', str(snapshots) + '/'),
                    ('cmd_rsync', shutil.which('rsync')), ('cmd_cp', '/bin/cp'),
                    ('cmd_rm', '/bin/rm'), ('retain', 'hourly\t24'), ('retain', 'daily\t7'),
                    ('retain', 'weekly\t4'), ('retain', 'monthly\t6'), ('sync_first', '1'),
                    ('rsync_short_args', '-a'),
            ...
            config.write_text(json.dumps({'rsnapshot': shutil.which('rsnapshot'),
                'rsnapshot_config': str(conf), 'snapshot_root': str(snapshots),
                'sqlite_stage': str(state / 'sqlite'), 'exclude_file': str(excludes)}))
