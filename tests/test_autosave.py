...
class AutosaveTests(unittest.TestCase):
    ...
    def test_terminal_status_and_small_ratio_noise_do_not_save(self):
        a = {'name':'Demo','layout':{'split':.50001,'title':'Codex busy','url':'https://example.com'}}
        b = copy.deepcopy(a);b['layout'].update(split=.50002,title='Codex idle')
    ...
    def test_conflict_preserves_local_revision(self):
        ...
        with patch.object(workspace,'snapshot_workspace',return_value={'revision':2}), patch.object(workspace,'remote',side_effect=RuntimeError('Workspace changed on another Mac')), patch.object(workspace,'store_instance') as store:
            with self.assertRaises(RuntimeError):workspace.save_state('demo',state,{})
    ...
    def test_two_changed_copies_pause_loser_without_overwrite(self):
        with tempfile.TemporaryDirectory() as tmp, patch.object(workspace,'STATE',Path(tmp)):
            for wid in ('one','two'):
                workspace.store('demo',{'workspace':wid,'bindings':{},'revision':1,'doc':{'id':'demo','name':'old'}})
            ...
            def snapshot(name,state,current):return dict(state['doc'],name=current['id'],revision=state['revision'])
            with patch.object(workspace,'cmux',return_value=trees), patch.object(workspace,'snapshot_workspace',side_effect=snapshot), patch.object(workspace,'remote',side_effect=rpc), patch.object(autosave,'notify'):
                d=autosave.Debounce();errors={}
                autosave.tick(d,errors,0);autosave.tick(d,errors,3);autosave.tick(d,errors,4)
